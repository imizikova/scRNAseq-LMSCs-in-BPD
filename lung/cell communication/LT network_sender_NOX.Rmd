...
```{r}
## receiver
receiver = c("gCap", "aCap", "Art", "Alv Mf", "Mono", "Int Mf", "B cell", "CD4+ T cell", "AT2", "AT1", "AT2-Lyz1+", "Ciliated", "Col13a1+ fibroblast", "Myofibroblast", "Col14a1+ fibroblast", "Pericyte 1", "Pericyte 2", "Ly6a+ L-MSC", "Mesothelial")
expressed_genes_receiver = get_expressed_genes(receiver, seuratObj, assay_oi = "RNA", pct = 0.20)
...
```{r}
write.csv(ligand_activities, file="C:/Users/.../ligand activities table - NOX_sender.csv", quote=F)
```

```{r}
ligand_activities <- read.csv("C:/Users/.../ligand activities table_NOX_sender.csv",row.names=1)
```
...
p_hist_lig_activity = ggplot(ligand_activities, aes(x=pearson)) + 
  ...
  geom_vline(aes(xintercept=min(ligand_activities %>% top_n(25, pearson) %>% pull(pearson))), color="red", linetype="dashed", size=1) + 
...
#- add this line if you want to filter out L_T couples with low weight
```{r}
active_ligand_target_links_df <- active_ligand_target_links_df[!rowSums(active_ligand_target_links_df < 0.0025),]
```
...
ggsave(p_ligand_target_network, filename="C:/Users/.../LT network_sender_NOX.png", width=5.3, height=4, dpi=600)
```
