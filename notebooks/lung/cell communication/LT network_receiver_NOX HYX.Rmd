...
seuratObj = readRDS("C:/Users/mizik/Desktop/IVANA/OHRI_2018_2020/L-MSCs_project/NEW SET OF DATA/Manuscript/scRNA seq data/Data/seurat_integrated_Ly6a L-MSC.rds")
...
```{r}
## sender
sender_celltypes = c("gCap", "aCap", "Art", "Neut 1", "Int Mf", "NK cell", "Col13a1+ fibroblast", "Ly6a+ L-MSC", "Mesothelial")
...
```{r}
write.csv(ligand_activities, file="C:/Users/.../ligand activities table - NOX/HYX_receivers.csv", quote=F)
```


```{r}
ligand_activities <- read.csv("C:/Users/.../ligand activities table - NOX/HYX_receivers.csv",row.names=1)
```


#The different ligand activity measures 
```{r}
best_upstream_ligands = ligand_activities %>% top_n(8, pearson) %>% arrange(-pearson) %>% pull(test_ligand) %>% unique()
```
...
p_hist_lig_activity = ggplot(ligand_activities, aes(x=pearson)) + 
  ...
  geom_vline(aes(xintercept=min(ligand_activities %>% top_n(8, pearson) %>% pull(pearson))), color="red", linetype="dashed", size=1) + 
...
ggsave(p_ligand_target_network, filename="C:/Users/.../LT network_receiver_NOX/HYX.png", width=4, height=3, dpi=600)
```
